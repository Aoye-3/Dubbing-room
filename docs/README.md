# Dubbing-room Documentation

本目录保存 Dubbing-room / VoxCPM-Box 的产品、技术、架构和实现计划文档。当前产品主线是基于 `IndexTTS2 + VoxCPM2` 的本地 AIGC 语音工作流配音室。

## 入口

- [PRD Index](PRD/README.md)
- [Technical Documentation Index](technical/README.md)
- [Historical IndexTTS 2.0 Runtime and UI Implementation Notes](technical/phase-1-2-runtime-ui-implementation.md)
- [App Development Docs](app-dev/README.md)
- [Dual-Model AppShell Execution Plan](Plan/11-dual-model-execution-plan.md)
- [IndexTTS-2.5 Replacement And Performance Desk Plan](Plan/12-indextts25-migration-plan.md)
- [IndexTTS-2.5 Upstream Contract Baseline](technical/indextts25-upstream-contract.md)
- [IndexTTS-2.5 Implementation Status](technical/indextts25-implementation-status.md)
- [Architecture Decision Records](decisions/)

## 最新重点

- [VoxCPM-Box AIGC 语音工作流配音室 PRD](PRD/aigc-voice-workflow-studio-prd.md)
- [ADR-001: Local Dual-Model Voice Studio](decisions/ADR-001-local-dual-model-voice-studio.md)
- [ADR-002: Safe GitHub Updates For Local AppShell](decisions/ADR-002-github-safe-local-updates.md)
- [ADR 0003: Hide promoted generation records from normal History](app-dev/adr/0003-generation-history-voice-promotion.md)
- [GitHub Safe Update Runbook](technical/github-safe-update-runbook.md)
- [双模型音频生成应用壳完整功能实现 PRD](PRD/dual-model-audio-appshell-full-implementation-prd.md)
- [系统架构](technical/architecture.md)
- [模型与运行时](technical/models-runtime.md)
- [存储与数据模型](technical/storage-data.md)
- [双模型互补应用壳 PRD](app-dev/10-dual-model-integration-prd.md)
- [ADR 0002: 双模型 RuntimeCoordinator](app-dev/adr/0002-dual-model-runtime-coordinator.md)
